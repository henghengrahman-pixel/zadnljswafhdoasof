...
def home():
    ...
    return jsonify({
        "status": "ok",
        "service": "deposit-monitor-sync-stable-v3.1",
...
def heartbeat():
    ...
    with db_lock, connect_db() as conn:
        # Bersihkan perangkat yang sudah tidak aktif lebih dari 7 hari.
        conn.execute(
        ...
        if not exists and MAX_DEVICES > 0:
            # Hanya hitung perangkat yang masih online.
            # Perangkat lama/offline tidak lagi memenuhi kuota.
            cutoff = now_ts - LEADER_TTL_SECONDS
            ...
            if active_total >= MAX_DEVICES:
                return jsonify({
                    "ok": False,
                    "error": (
                        f"Batas perangkat online tercapai ({MAX_DEVICES}). "
                        "Matikan salah satu perangkat atau set MAX_DEVICES=0."
                    )
                }), 403
