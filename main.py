...
def form_alert():
    ...
    with db_lock, db() as conn:
        ...
        conn.execute(
            """
            INSERT INTO sent_forms(form_id, username, game_id, form_time, amount, bank, sent_at, device_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (form_id, username, game_id, form_time, amount, bank, now_ts, device_id)
        )
        conn.commit()

    message = (
        "⚠️ <b>FORM DEPOSIT TERLAMBAT</b>\n\n"
        f"{escape(game_id)} ID: <b>{escape(username)}</b>\n"
